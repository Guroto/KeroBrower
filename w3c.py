...
class Node(PyV8.JSClass):
    ...
    def href(self):
        ...
        r = re.findall(pattern, self.__str__())
        if r and r[0] == '/':
            l = self.doc.URL.split('/')
            return l[0] + '//' + l[2]
        return r[0] if r else None

    @href.setter
    def href(self, value):
        self._parent.tag.string = re.sub(r'href=\'(.*)\'', "href='{}'".format(value), self.tag.string)
        return self._parent.tag.string
...
class Element(Node):
    ...
    @property
    def firstChild(self):
        r = Node.wrap(self.doc, self.tag.contents[0]) if len(self.tag) > 0 else None
        r._parent = self
        return r
...
class Document(Node):
    ...
    def attachEvent(self, type, handler):
        if type == 'onreadystatechange' or 'DOMContentLoaded':
            _handler = str(handler)
            _handler2 = re.sub(r'''setTimeout.*,1500\);''', '', _handler)
            script = 'var l=' + _handler2 + ';l();'
            self.window.eval(script)
        else:
            print type, handler
            ...
            #     handler()
#
class XMLHttpRequestUpload(PyV8.JSClass):
    pass
...
class XMLHttpRequest(PyV8.JSClass):
    ...
    def open(self, method, url, statu=False):
...
class HTMLElement(Element, ElementCSSInlineStyle):
    ...
    def innerHTML(self, html):
        self.tag.string = str(html.encode('utf-8'))
        # print 'html', bs4.BeautifulSoup(html, 'lxml')
        # print 'self.tag', self.tag, type(self.tag)
...
class HTMLDocument(Document):
    ...
    head = xpath_property("/html/head[1]")
    ...
    def lastModified(self):
        self._lastModified = time.strftime('%m/%d/%Y %H:%M:%S', time.localtime())
    ...
    def close(self):
        ...
        self.doc = bs4.BeautifulSoup(html, 'lxml')
    ...
    def write(self, html):
        if self._html:
            ...
            for tag in bs4.BeautifulSoup(html, 'lxml').contents:
...
def parseString(html, **kwds):
    return DOMImplementation(bs4.BeautifulSoup(html, 'lxml'), **kwds)
